...
app.get('/user/login', async (req, res) => {
  console.log("/user/login 진입");
  const { idd_p1, pwdd_p2 } = req.query;
  console.log("넘어온 파라미터들" + idd_p1 + " " + pwdd_p2);
  try {
    const result = await connection.execute(
      	  `SELECT * FROM TBL_USER `
					 + ` WHERE USERID = '${idd_p1}' `
					 + ` AND PASSWORD = '${pwdd_p2}' `
    );
    console.log(result);
    const columnNames = result.metaData.map(column => column.name);
    // 쿼리 결과를 JSON 형태로 변환
    ...
    // 디버깅 로그 전경환 추가--------------------S
    console.log('login rows.length =', result.rows.length);
